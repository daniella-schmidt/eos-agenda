<x-app-layout>
    <div class="min-h-[calc(100vh-4rem)] bg-[#f6fbfb]">
        <style>
            .events-page { max-width: 1440px; margin: 0 auto; padding: 24px; }
            ...
            .event-suggestions-list { display: grid; gap: 10px; }

            .event-suggestion-item {
                border: 1px solid #dbe7e7;
                border-left: 5px solid #ff6bb3;
                border-radius: 8px;
                background: #fff;
                padding: 12px;
            }

            .event-suggestion-item.is-selected { border-left-color: #008f91; background: #f3ffff; }

            .event-suggestion-item__title { color: #0d2b2b; font-size: .92rem; font-weight: 900; }
            .event-suggestion-item__meta { margin-top: 4px; color: #647878; font-size: .8rem; font-weight: 700; }
            .event-suggestion-item__reason { margin-top: 8px; color: #526767; font-size: .82rem; font-weight: 600; line-height: 1.5; }
            ...
            .status-pill.is-high { background: #fff1f7; color: #b42369; }

            @media (max-width: 1050px) {
                .events-workspace, .events-grid { grid-template-columns: 1fr; }
            }
        </style>
        ...
        <script>
            ...
            async function loadEventSuggestions() {
                ...
                smartRequests = requestResponses
                    .flatMap(r => r.data || [])
                    .filter(r => { if (requestMap.has(r.id)) return false; requestMap.set(r.id, r); return true; });

                const suggestionResponses = await Promise.all(
                    smartRequests.map(req =>
                        api(`/api/smart-requests/${req.id}/suggestions`)
                            .then(r => ({ request: req, suggestions: r.data || [] }))
                    )
                );
                        ...
                        <article class="event-suggestion-item ${suggestion.selected ? 'is-selected' : ''}">
                            <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px;">
                                <div>
                                    ...
                                    <div class="event-suggestion-item__meta">
                                        Score ${Number(suggestion.score || 0).toFixed(1)}
                                    </div>
                                </div>
                            ...
                            <div class="events-actions mt-3">
                                <button class="events-btn events-btn--ghost" type="button"
                                        data-action="use-suggestion" data-suggestion-id="${suggestion.id}">
                                    Usar horário
                                </button>
                            </div>
                        </article>
            ...
            async function useEventSuggestion(id) {
                ...
                const response = await api(`/api/event-suggestions/${id}/select`, { method: 'POST' });

                eventSuggestions = eventSuggestions.map(s => ({ ...s, selected: String(s.id) === String(id) }));
                renderEventSuggestions();

                const selected = response.data || suggestion;
            ...
            async function syncChildren(eventId) {
                await Promise.all(selectedParticipants.map(async p => {
                ...
                await Promise.all(selectedReminders.map(r =>
            ...
            function setDefaultDates() {
                let start;
                if (dateFromUrl) {
                    // data vinda do calendário: usa o dia clicado às 14h
            ...
            loadBootstrap()
                .then(() => {
                    ...
                    if (calendarIdFromUrl) {
                        el('calendarId').value = calendarIdFromUrl;
                    }
                    renderParticipants();
                    renderReminders();
                    return loadEventSuggestions();
                })
                .then(() => {
                    if (suggestionIdFromUrl) {
                        useEventSuggestion(suggestionIdFromUrl);
                    }
                })
